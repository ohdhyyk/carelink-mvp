...
def all_received_done_for_day(acc: int, day: date) -> bool:
    \"\"\"
    A day is considered 'done' for a user if:
      - They have at least one received task that existed on/before that day, AND
      - All those received tasks are marked done for that day.

    IMPORTANT: If there are ZERO received tasks for that day, we return False.
    This prevents an infinite streak when the app has no tasks yet.
    \"\"\"
...
def pair_streak(up_to: date) -> int:
    \"\"\"
    Consecutive days (ending at up_to) where BOTH users completed ALL their received tasks.

    Safety guards:
      - Maximum lookback window to avoid date underflow / infinite loops.
    \"\"\"
    s = 0
    d = up_to

    # Look back at most 366 days (demo-safe)
    for _ in range(366):
...
if not received:
    st.caption("No tasks received from your linked user today. (Streak won’t increase when there are no received tasks.)")
else:
